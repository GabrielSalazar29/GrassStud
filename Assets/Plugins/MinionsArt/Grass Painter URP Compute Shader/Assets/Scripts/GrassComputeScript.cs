...
public class GrassComputeScript : MonoBehaviour {
	...
	// interactors
	GrassInteractor[] interactors;

	// base data lists
	[SerializeField, HideInInspector]
	List<GrassData> grassData = new();

	// list of all visible grass ids, rest are culled
	List<int> grassVisibleIDList = new();
	...
	private readonly uint[] argsBufferReset = new uint[5] {
	...
	private readonly List<Bounds> BoundsListVis = new();
	private readonly List<CullingTreeNode> leaves = new();
	private readonly Plane[] cameraFrustumPlanes = new Plane[6];
	private float cameraOriginalFarPlane;

	// list of -1 to overwrite the grassvisible buffer with
	private List<int> empty = new();
	...
	private void SetGrassDataBase(bool full) {
		...
		if (full) {
			...
			interactors = (GrassInteractor[])FindObjectsOfType(typeof(GrassInteractor));
		}
		else {
	...
	void OnDrawGizmos() {
		if (drawPoints) {
			...
			for (int i = 0; i < grassData.Count; i++)
				Gizmos.DrawSphere(grassData[i].position, pointRadius);
		}

		if (currentPresets != null && currentPresets.drawBounds) {
